(function ($) {
    ...
    __center_canvas = function(){
        ...
        if (canvas_wrapper_left_margin < 50) {
            canvas_wrapper_left_margin = 50;
        }
        if (canvas_wrapper_top_margin < 50) {
            canvas_wrapper_top_margin = 50;
        }
        jQuery('.canvas-container').css({
            'margin-top': canvas_wrapper_top_margin,
            'margin-left': canvas_wrapper_left_margin
        });
        //And then center the ruler
        var _left_margin = ((jQuery('[data-udraw="canvasWrapper"]').width() - jQuery('[data-udraw="canvas"]').width()) / 2) - (25 * 2);
        var _top_margin = ((jQuery('[data-udraw="canvasWrapper"]').height() - jQuery('[data-udraw="canvas"]').height()) / 2) - (25 * 2);
        if (_left_margin < 0) {
            _left_margin = 0;
        }
        if (_top_margin < 0) {
        ...
        jQuery('[data-udraw="topRuler"], [data-udraw="sideRuler"]').css({
            'margin-top': _top_margin,
            'margin-left': _left_margin
        });
    }
})(window.jQuery);